...
required is:
    ...
    • Main XML file of the recording. This is the base file name and will match the modality of the imaging recording if present.
    ...
    • XML file of the VoltageRecording (Usually titled _Cycle00001_VoltageRecording_001.xml). This contains information
      relating to the configuration of the amplifier as well as number of samples and date information.
...
class PATCH:
    ...
    def __init__(self,
                 main_xml_file_path: Union[str, pathlib.Path],
                 load_data: bool = True):
        """
        Load XML and CSV data from their respective files.
        ### Parameters
        1. main_xml_file_path -- Path to the main xml file.
        2. load_data -- Load the CSV and XML data files.
        """

        if isinstance(main_xml_file_path, pathlib.Path):
            patch_csv_file_path = str(main_xml_file_path)

        if main_xml_file_path.lower() and not main_xml_file_path.endswith('.xml'):
            raise Exception("Patch file must be a CSV!")

        if os.path.isdir(main_xml_file_path):
            raise Exception("CSV path must be a path to a FILE not a FOLDER.")

        self._preLoadData = load_data

        # TODO: Convert everything to pull in the base XML, grab the file names, and import using those
